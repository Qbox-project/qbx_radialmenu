...
function ToggleProps(whic)
    ...
    if not Prop.Variants then
        ...
        lib.notify({
            description = Lang:t("info.nothing_to_remove")
        })
...
function IsMpPed(ped)
    local male = joaat('mp_m_freemode_01')
    local female = joaat('mp_f_freemode_01')
    ...
    for _, v in pairs(LastEquipped) do
        if v then
            if v.Drawable then
                SetPedComponentVariation(cache.ped, v.ID, v.Drawable, v.Texture, 0)
            elseif v.Prop then
                ClearPedProp(cache.ped, v.ID) SetPedPropIndex(cache.ped, v.ID, v.Prop, v.Texture, true)
            end
        end
    end
