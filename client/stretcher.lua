...
local function setClosestStretcher()
    ...
    local object = GetClosestObjectOfType(coords.x, coords.y, coords.z, 10.0, joaat('prop_ld_binbag_01'), false, false, false)
...
RegisterNetEvent('qb-radialmenu:client:TakeStretcher', function()
    ...
    if vehicle ~= 0 then
        lib.requestModel("prop_ld_binbag_01")

        local obj = CreateObject(joaat('prop_ld_binbag_01'), GetEntityCoords(cache.ped), true)
...
RegisterNetEvent('qb-radialmenu:Stretcher:client:BusyCheck', function(otherId, type)
    if type == "lay" then
        loadlib.requestAnimDictAnim("anim@gangops@morgue@table@")
...
CreateThread(function()
    while true do
        ...
        if isAttached then
            ...
            if IsPedShooting(ped) or IsPlayerFreeAiming(cache.playerId) or IsPedInMeleeCombat(cache.ped) or IsEntityDead(cache.ped) or IsPedRagdoll(cache.ped) then
                detachStretcher()
            end
        end
...
CreateThread(function()
    ...
    local pos = GetEntityCoords(cache.ped)
    local object = GetClosestObjectOfType(pos.x, pos.y, pos.z, 5.0, joaat('prop_ld_binbag_01'), false, false, false)
